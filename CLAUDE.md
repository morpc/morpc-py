...
- **CSV line endings**: Always `\r\n` (Windows). `write_table()` enforces this via `PANDAS_EXPORT_ARGS_OVERRIDE`. This ensures consistent MD5 checksums cross-platform.
  Git may still check a CSV out with LF endings (e.g. `* text=auto` on Linux), so `_verify_hash()` and `validate_resource()` accept a CSV whose hash/bytes match with either line ending, including CRLF at record ends only (pandas on Windows leaves line breaks inside quoted values as LF); the file on disk is never rewritten.
